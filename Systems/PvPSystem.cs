...
namespace PvPModes.Systems
{
    public class PvPSystem
    {
        ...
        public static void MobKillMonitor(Entity KillerEntity, Entity VictimEntity)
        {
            ...
            {
                P_Cache.ReputationLog.TryGetValue(killer_id, out var RepLog);
                ...
                P_Cache.ReputationLog[killer_id] = RepLog;
            }
        ...
        public static void Monitor(Entity KillerEntity, Entity VictimEntity)
        {
            ...
            if (isHonorSystemEnabled)
            {
                ...
                if (RepLog.TotalGained < MaxHonorGainPerSpan)
                {
                    ...
                    {
                        P_Cache.HostilityState.TryGetValue(VictimEntity, out var state);
        ...
        public static bool HostileON(ulong SteamID, Entity playerEntity, Entity userEntity)
        {
            ...
            P_Cache.HostilityState[playerEntity] = stateData;
        ...
        {
            if (seekAlly)
			{ /* ///COMMENTED OUT TO FIX ---THAZZEUS
                if (Helper.GetAllies<--- THE ISSUE, THAZZEUS (playerEntity, out var playerGroup) > 0)
                {
                    ...
                    if (forceSiege == false)
                    {
                        ...
                        {
                            P_Cache.HostilityState.TryGetValue(ally.Value, out var hostilityState);
        ...
        public static void OnEquipChange(Entity player)
        {
            ...
            P_Cache.PlayerLevelP_Cache.TryGetValue(player, out var levelData);

            if (PlayerLevel > levelData.Level)
            {
                levelData.Level = PlayerLevel;
                levelData.TimeStamp = DateTime.Now;
                P_Cache.PlayerLevelP_Cache[player] = levelData;
            }
        }
        ...
        public static void PunishCheck(Entity Killer, Entity Victim)
        {
            ...
            float KillerLevel;
            if (P_Cache.PlayerLevelP_Cache.TryGetValue(Killer, out var killerData)) KillerLevel = killerData.Level;
            else KillerLevel = em.GetComponentData<Equipment>(Killer).GetFullLevel();

            float VictimLevel;
            if (P_Cache.PlayerLevelP_Cache.TryGetValue(Victim, out var victimData)) VictimLevel = victimData.Level;
            else VictimLevel = em.GetComponentData<Equipment>(Victim).GetFullLevel();

            if (VictimLevel - KillerLevel <= PunishLevelDiff)
            {
                P_Cache.OffenseLog.TryGetValue(KillerSteamID, out var OffenseData);
        ...
        public static void HonorBuffReceiver(Entity BuffEntity, PrefabGUID GUID)
        {
            ...
            if (P_Cache.HostilityState.TryGetValue(HBOwner, out var stateData))
            {
        ...
        public static void NewPlayerReceiver(Entity userEntity, Entity playerEntity, FixedString64 playerName)
        {
            ...
            else
            {
                ...
                Helper.UpdatePlayerP_Cache(userEntity, recastName, recastName, false);
            }

            var steamID = Plugin.Server.EntityManager.GetComponentData<User>(userEntity).PlatformId;
            P_Cache.HostilityState[playerEntity] = new StateData(steamID, false);
        }
